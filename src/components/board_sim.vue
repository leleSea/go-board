        ...
        <le-confirm ref="downloadSgfPanel">
            <template v-slot:message>
                <div class="download-sgf-box">
                    <div class="upload-title">
                        <span class="txt">{{$t('保存到本地')}}</span>
                    </div>
                    <table class="down-load-table"> 
                        <tr>
                            <td>
                                <span class="txt">{{$t('文件名')}}</span>
                            </td>
                            <td colspan="5">
                                <div class="file-name">
                                    <el-input v-model="sgfInfoTemplate.fileName.value"></el-input>
                                    <span class="txt">{{$t('.sgf')}}</span>
                                </div>
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <span class="txt">{{$t('棋谱')}}</span>
                            </td>
                            <td colspan="5">
                        ...
                        <tr>
                            ...
                            <td>
                                <span class="txt">{{$t('比赛用时')}}</span>
                            </td>
                            <td>
                                <el-input v-model="sgfInfoTemplate.tm.value"></el-input>
                            </td>
                            <td>
                                <span class="txt">{{$t('回合')}}</span>
                            </td>
                            <td>
...
export default {
    ...
    methods: {
        ...
        sgfInfoInit(){
            let sgfInfo = this.GLOBAL.deepCopy(this.GLOBAL.sgfInfo)
            sgfInfo.km.label = this.$i18n.t(sgfInfo.km.label)
            sgfInfo.fileName = {label: '文件名', value: ''}
        ...
        downloadSgfConfirm(){
            if(this.board.position <= 0){
                this.errorNotice(this.$i18n.t('当前盘面为空'))
                return
            }
            let sgfInfo = this.GLOBAL.deepCopy(this.sgfInfoTemplate)
            if(!sgfInfo.fileName.value || sgfInfo.fileName.value == ''){
                this.errorNotice(this.$i18n.t('请输入文件名'))
                return
            }
            let options = {
                sgfInfo: sgfInfo,
